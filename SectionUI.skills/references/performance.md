...
// 清除特定 ID 的所有 limit 缓存
sizeStore.remove(by: "item-123")
...
section.prefetch.prefetchPublisher
    .sink { [weak self] rows in
        self?.preloadData(at: rows)
    }
    .store(in: &cancellables)

section.prefetch.cancelPrefetchingPublisher
    .sink { [weak self] rows in
        self?.cancelPreloading(at: rows)
    }
    .store(in: &cancellables)
```
...
class ImageListViewController: SKCollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        section.prefetch.cancelPrefetchingPublisher
            .sink { [weak self] rows in
                rows.forEach { row in
                    self?.cancelImageLoad(at: row)
                }
            }
            .store(in: &cancellables)
    }
    
    private func preloadImage(at row: Int) {
        ...
        loadingTasks[model.imageURL] = task
        task.resume()
    }
    
    private func cancelImageLoad(at row: Int) {
        guard models.indices.contains(row) else { return }
        let key = models[row].imageURL
...
// 固定宽高比：先算宽度，再由宽度推导高度
section.cellSafeSize(.fraction(1.0 / 3.0), transforms: .height(asRatioOfWidth: 9.0 / 16.0))

// 组合多个转换
section.cellSafeSize(
    .fraction(0.5),
    transforms: [
        .height(asRatioOfWidth: 1.0),  // 正方形
        .offset(width: -8),
        .offset(height: -8)
    ]
)
...
```swift
// 两列，等宽，按照模型中的宽高比设置测量高度
let imageRatio = model.imageSize.height / max(model.imageSize.width, 1)

section.cellSafeSize(
    .fraction(0.5),
    transforms: .height(asRatioOfWidth: imageRatio)
)
```
...
section.model(displayedAt: [1, 3, 5]) { context in
    print("row \(context.row) displayed at selected counts")
}
```
...
```swift
// ✅ 推荐 - 复杂布局
class ComplexCell: UICollectionViewCell, SKLoadViewProtocol, SKConfigurableAutoAdaptiveView {
    // 大量 Auto Layout 约束...
}
