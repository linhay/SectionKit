...
private extension SKCSingleTypeSection {
    ...
    func reload(models: [Model], by areEquivalent: (Model, Model) -> Bool) {
        ...
        guard let sectionView = sectionInjection?.sectionView else {
            self.models = models
            return
        }

        let difference = models.difference(from: oldModels, by: areEquivalent)
        guard !difference.isEmpty else {
            self.models = models
            sectionInjection?.reload(cell: indexsForVisibleItems.filter { models.indices.contains($0) })
            return
        }
        ...
        for change in difference {
            ...
            case .remove(let offset, _, _):
                deleteRows.append(offset)
            case .insert(let offset, _, _):
                insertRows.append(offset)
            }
        }
        ...
        for row in deleteRows where oldModels.indices.contains(row) {
            deletedModels[row] = oldModels[row]
        }

        let deleteIndexPaths = deleteRows.map(indexPath(from:))
        let insertIndexPaths = insertRows.map(indexPath(from:))
        sectionView.performBatchUpdates {
            self.models = models
            if !deleteIndexPaths.isEmpty {
                sectionView.deleteItems(at: deleteIndexPaths)
            }
            if !insertIndexPaths.isEmpty {
                sectionView.insertItems(at: insertIndexPaths)
            }
        } completion: { _ in
            self.sectionInjection?.reload(cell: self.indexsForVisibleItems.filter { self.models.indices.contains($0) })
        }
    }
