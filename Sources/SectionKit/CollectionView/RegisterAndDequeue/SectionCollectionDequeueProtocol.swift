...
public extension SectionCollectionDequeueProtocol {
    ...
    func register<T: UICollectionViewCell & SectionLoadViewProtocol>(_: T.Type) {
        if let nib = T.nib {
    ...
    func register<T: UICollectionReusableView & SectionLoadViewProtocol>(_: T.Type, for kind: SectionSupplementaryKind) {
        if let nib = T.nib {
