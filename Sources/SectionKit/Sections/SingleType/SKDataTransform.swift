...
open class SKDataTransform<Model> {
...
public class SKHiddenTransform<Model>: SKDataTransform<Model> {
...
public class SectionTransforms<Cell: SKConfigurableModelProtocol> {
    public let hidden = SKHiddenTransform<Cell.Model>(task: nil)
    public let validate = SKDataTransform<Cell.Model>(task: { $0.filter(Cell.validate) })
